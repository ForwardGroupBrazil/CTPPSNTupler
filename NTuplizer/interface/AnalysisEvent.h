...
namespace ggll
{
  ...
  class AnalysisEvent
  {
	...
	if ( HLT_Name ) delete HLT_Name;
      ...
      static constexpr unsigned int MAX_LL = 25;
      ...
      static constexpr unsigned int MAX_PHO = 25;
      ...
      static constexpr unsigned int MAX_JETS = 10;
      /// Maximum number of jets per event
      static constexpr unsigned int MAX_PF = 80;
	...
	int HLT_LeadingLepton_Prescl[MAX_HLT], HLT_TrailingLepton_Prescl[MAX_HLT];*/
      ...
      double PFCand_pt[MAX_PF], PFCand_eta[MAX_PF], PFCand_phi[MAX_PF], PFCand_e[MAX_PF], PFCand_vz[MAX_PF];
      int PFCand_charge[MAX_PF], PFCand_id[MAX_PF];
      double Mpf, SumPF_e;
	...
	// PF collection
	nPFCand = 0;
	for ( unsigned int i = 0; i < MAX_PF; ++i ) {
	  PFCand_pt[i] = PFCand_eta[i] = PFCand_phi[i] = PFCand_e[i] = PFCand_charge[i] = PFCand_id[i] = PFCand_vz[i] = -999;
	}
	Mpf=0.;
        SumPF_e=0.;
	...
	tree->Branch( "PFCand_id", PFCand_id, "PFCand_id[nPFCand]/I" );
	tree->Branch( "PFCand_vz", PFCand_vz, "PFCand_vz[nPFCand]/D" );
	tree->Branch( "PFCand_pt", PFCand_pt, "PFCand_pt[nPFCand]/D" );
	tree->Branch( "PFCand_eta", PFCand_eta, "PFCand_eta[nPFCand]/D");
	tree->Branch( "PFCand_phi", PFCand_phi,  "PFCand_phi[nPFCand]/D");
	tree->Branch( "PFCand_e", PFCand_e,  "PFCand_e[nPFCand]/D" );
	tree->Branch( "PFCand_charge", PFCand_charge,"PFCand_charge[nPFCand]/I" );
	tree->Branch( "Mpf", &Mpf, "Mpf/D");
        tree->Branch( "SumPF_e", &SumPF_e, "SumPF_e/D");
	...
	tree->SetBranchAddress( "PFCand_id", PFCand_id );
	tree->SetBranchAddress( "PFCand_vz", PFCand_vz );
	...
	tree->SetBranchAddress( "PFCand_e", PFCand_e );
	tree->SetBranchAddress( "PFCand_charge", PFCand_charge );
	tree->SetBranchAddress( "Mpf", &Mpf );
        tree->SetBranchAddress( "SumPF_e", &SumPF_e);
