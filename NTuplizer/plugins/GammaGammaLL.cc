  ...
  nCandidates_( 0 )
{
  ...
  //consumes<vector<reco::PFCandidate> >(edm::InputTag("particleFlow"));
...
void GammaGammaLL::fetchPF( const edm::Event& iEvent)
{

  // Fill Particle Flow
  edm::Handle <vector<reco::PFCandidate>> PFCandidates;
  iEvent.getByToken(pfToken_,PFCandidates);
  reco::PFCandidate::const_iterator iter;

  double totalPFEnergy=0;
  math::XYZTLorentzVector allCands(0.,0.,0.,0.);
  for( unsigned int i = 0; i < PFCandidates->size() && evt_.nPFCand < ggll::AnalysisEvent::MAX_PF; ++i ) {
    ...
    { 
      allCands+=pfAll->p4();
      evt_.PFCand_id[evt_.nPFCand] = pfAll->particleId();
      evt_.PFCand_vz[evt_.nPFCand] = pfAll->vz();
      evt_.PFCand_pt[evt_.nPFCand] = pfAll->pt();
      evt_.PFCand_eta[evt_.nPFCand] = pfAll->eta();
      evt_.PFCand_phi[evt_.nPFCand] = pfAll->phi();
      evt_.PFCand_e[evt_.nPFCand] = pfAll->energy();
      totalPFEnergy += evt_.PFCand_e[evt_.nPFCand];
      evt_.PFCand_charge[evt_.nPFCand] = pfAll->charge();
      evt_.nPFCand++;
    }
...
GammaGammaLL::fetchJets( const edm::Event& iEvent )
{
  ...
  for ( unsigned int i = 0; i < jetColl->size() && evt_.nJetCand < ggll::AnalysisEvent::MAX_JETS; ++i ) {
    ...
    if ( evt_.JetCand_pt[evt_.nJetCand] > HEJet_pt ) {
