...
//1 charged (trk)
//2 electron (ch+ECAL)
//3 electron (trk+electron chamber)
...
void DarkMatterSearchLeadingElectrons(string year, vector<string> const& fileNames, string const& outputFileNameTree = "TTreeOutput.root",const Int_t nevt_max = 100)
{ 
  ...
  double EleCand_et[100];
  double EleCand_px[100]; 
  double EleCand_py[100];
  double EleCand_pz[100];
  int EleCand_MVAtightID[100];
  double EleCand_eta[100];
  double EleCand_phi[100];
  double EleCand_e[100];
  int EleCand_charge[100];

  double LocalProtCand_x[100];
  double LocalProtCand_z[100];

  int nPixelTracks=0;
  double PixTrackX[100];
  double PixTrackY[100];
  int PixTrackArm[100];
  ...
  for(vector<TString>::iterator itfiles = vfiles->begin() ; itfiles != vfiles->end() && i_tot < nevt_max_corr ; ++itfiles){
    ...
    if(year=="2016"){
      ...
      tree->SetBranchAddress("EleCand_MVAtightID",&EleCand_MVAtightID);    
    }else{
      exit (EXIT_FAILURE);
    }
    ...
    double LeadingElectronEt=0;
    double SecondElectronEt=0;
    double LeadingElectronPt=0.;
    double SecondElectronPt=0.;
    double LeadingElectronVtxZ=0.;
    double SecondElectronVtxZ=0.;
    double VtxZ=0.;
    double VtxZpps=0.;

    int LeadingElectronMVAtightID=0;
    int SecondElectronMVAtightID=0;
    double Electrons_pxtot=0.;
    double Electrons_pytot=0.;
    double Electrons_pztot=0.;
    double Electrons_ettot=0.;
    double pairMass=0.;
    double pairPhi=0.;
    double pairEta=0.;
    double pairpT=0.;
    ...
    trout->Branch("nEleCand",&nEleCand,"nEleCand/I");
    trout->Branch("LeadingElectronEt",&LeadingElectronEt,"LeadingElectronEt/D");
    trout->Branch("SecondElectronEt",&SecondElectronEt,"SecondElectronEt/D");
    trout->Branch("LeadingElectronVtxZ",&LeadingElectronVtxZ,"LeadingElectronVtxZ/D");
    trout->Branch("SecondElectronVtxZ",&SecondElectronVtxZ,"SecondElectronVtxZ/D");
    ...
    if(year=="2017"){
      trout->Branch("LeadingElectronMVAtightID",&LeadingElectronMVAtightID,"LeadingElectronMVAtightID/I");
      trout->Branch("SecondElectronMVAtightID",&SecondElectronMVAtightID,"SecondElectronMVAtightID/I");
	...
	loadBar(i_evt, nev);
      ...
      for(int i=0;i<150;i++){
	pixelcoordX[0][i]=0;pixelcoordX[1][i]=0;
	pixelcoordY[0][i]=0;pixelcoordY[1][i]=0;
      }
      ...
      LeadingElectronEt=0;
      SecondElectronEt=0;
      LeadingElectronMVAtightID=0;
      SecondElectronMVAtightID=0;
      LeadingElectronVtxZ=0;
      SecondElectronVtxZ=0;
      VtxZ=0;
      VtxZpps=0;
      pairMass=0.;
      pairPhi=0.;
	...
	NpixelLL=0;
	NpixelRR=0;
	for(int s=0;s<nPixelTracks;s++)
	{
	  if(PixTrackArm[s]==0){
	    pixelcoordX[0][NpixelLL]=PixTrackX[s]; 
	    pixelcoordY[0][NpixelLL]=PixTrackY[s]; 
	    NpixelLL++; 
	    pixelLL=s; 
	    //nLocalProtCand++;
	  }
	  if(PixTrackArm[s]==1){
	    pixelcoordX[1][NpixelRR]=PixTrackX[s]; 
	    pixelcoordY[1][NpixelRR]=PixTrackY[s]; 
	    NpixelRR++;
	    pixelRR=s; 
	    //nLocalProtCand++;
	  }
	  if(debug){
	    std::cout << "pixel X [mm]: " << PixTrackX[s] << ", ARM: " << PixTrackArm[s] << std::endl;
	    std::cout << "pixel Y [mm]: " << PixTrackY[s] << ", ARM: " << PixTrackArm[s] << std::endl;
	  }
	}
	...
	if(nLocalProtCand==2)
	{
	  ...
	  TLorentzVector electron0;  
	  TLorentzVector electron1;
	  TLorentzVector electronpair;
	  ...
	  if(protonsAccept){
	    if(nEleCand>1){
	      ...
	      LeadingElectronEt=EleCand_et[0];
	      SecondElectronEt=EleCand_et[1];
	      LeadingElectronVtxZ=EleCand_vtxz[0];
	      SecondElectronVtxZ=EleCand_vtxz[1];
	      VtxZ=KalmanVertexCand_z[0];

	      electron0.SetPtEtaPhiE(EleCand_et[0], EleCand_eta[0], EleCand_phi[0], EleCand_e[0]);
	      electron1.SetPtEtaPhiE(EleCand_et[1], EleCand_eta[1], EleCand_phi[1], EleCand_e[1]);
	      electronpair = electron0+electron1;

	      Electrons_Etot=sqrt(EleCand_e[0]*EleCand_e[0]+MASS_E*MASS_E) + sqrt(EleCand_e[1]*EleCand_e[1]+MASS_E*MASS_E);
	      Electrons_pxtot = electron0.Px() + electron1.Px();
	      Electrons_pytot = electron0.Py() + electron1.Py();
	      Electrons_pztot = electron0.Pz() + electron1.Pz();
	      chargeVal=(int)EleCand_charge[0]+(int)EleCand_charge[1];

	      pairMass=electronpair.M();
	      pairEta=electronpair.Eta();
	      pairPhi=electronpair.Phi();
	      pairpT=electronpair.Pt();

	      double dphi = fabs(electron0.Phi()-electron1.Phi());
	      dphi = (dphi<pi) ? dphi : 2.*pi-dphi;
	      acoplan = 1 - fabs(dphi)/pi;

	      missingmass=sqrt((ECM-(Electrons_Etot+fabs(proton1_pz)+fabs(proton2_pz)))*(ECM-(Electrons_Etot+fabs(proton1_pz)+fabs(proton2_pz)))-(Electrons_pxtot)*(Electrons_pxtot)-(Electrons_pytot)*(Electrons_pytot)- (Electrons_pztot+proton1_pz+proton2_pz)*(Electrons_pztot+proton1_pz+proton2_pz));

	      ppZsystem = p1 + p2 - electronpair;
	      missingmassLorentz = ppZsystem.M();

	      if(year=="2017"){

		LeadingElectronMVAtightID=EleCand_MVAtightID[0];
		SecondElectronMVAtightID=EleCand_MVAtightID[1];
...
int main(int argc, char** argv)
{
  ...
  if(year_str=="2016"){
    ...
    if(era_str=="B")allnamesWithPrefixFinal.push_back("/eos/cms/store/group/phys_pps/dilepton/DoubleEG/Data-RunB/DataRunB.root");
    if(era_str=="C")allnamesWithPrefixFinal.push_back("/eos/cms/store/group/phys_pps/dilepton/DoubleEG/Data-RunC/DataRunC.root");
    if(era_str=="G")allnamesWithPrefixFinal.push_back("/eos/cms/store/group/phys_pps/dilepton/DoubleEG/Data-RunG/DataRunG.root");
    ...
    //if(era_str=="B")allnamesWithPrefixFinal.push_back("/eos/cms/store/user/kshcheli/EE17/ee/DoubleEG/EErunB.root");
    if(era_str=="B")allnamesWithPrefixFinal.push_back("/eos/cms/store/group/phys_pps/MissingMassSearch/NTuples/ElectronB.root");
    if(era_str=="C")allnamesWithPrefixFinal.push_back("/eos/cms/store/user/kshcheli/EE17/ee/DoubleEG/EErunC.root");
    if(era_str=="D")allnamesWithPrefixFinal.push_back("/eos/cms/store/user/kshcheli/EE17/ee/DoubleEG/EErunD.root");
    if(era_str=="E")allnamesWithPrefixFinal.push_back("/eos/cms/store/user/kshcheli/EE17/ee/DoubleEG/EErunE.root");
    if(era_str=="F")allnamesWithPrefixFinal.push_back("/eos/cms/store/user/kshcheli/EE17/ee/DoubleEG/EErunF.root");
  }else{
  ...
  DarkMatterSearchLeadingElectrons(year_str, allnamesWithPrefixFinal, filenameTree, maxevent);
