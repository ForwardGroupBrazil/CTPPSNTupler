...
void DarkMatterSearchLeadingElectronsJets(string year, vector<string> const& fileNames, string const& outputFileNameTree = "TTreeOutput.root",const Int_t nevt_max = 100)
{ 
	...
	if(nLocalProtCand==2)
	{
	  ...
	  if(protonsAccept){
	    if(nEleCand>1  && nJetCand>1){
	      ...
	      dileptonMass=electronpair.M();
	      dileptonEta=electronpair.Eta();
	      dileptonPhi=electronpair.Phi();
	      dileptonpT=electronpair.Pt();
	      ...
	      ppZsystem = p1 + p2 - (jetpair + electronpair);
	      missingmassLorentz = ppZsystem.M();

	      if(year=="2017"){

		LeadingElectronMVAtightID=EleCand_tightID[0];
		SecondElectronMVAtightID=EleCand_tightID[1];
		MeT = Etmiss;
...
int main(int argc, char** argv)
{
  ...
  if(year_str=="2016"){
    ...
    if(era_str=="C")allnamesWithPrefixFinal.push_back("/eos/cms/store/group/phys_pps/MissingMassSearch/NTuples/ElectronC.root");
    if(era_str=="D")allnamesWithPrefixFinal.push_back("/eos/cms/store/group/phys_pps/MissingMassSearch/NTuples/ElectronD.root");
    if(era_str=="E")allnamesWithPrefixFinal.push_back("/eos/cms/store/group/phys_pps/MissingMassSearch/NTuples/ElectronE.root");
    if(era_str=="F")allnamesWithPrefixFinal.push_back("/eos/cms/store/group/phys_pps/MissingMassSearch/NTuples/ElectronF.root");
  }else{
  ...
  DarkMatterSearchLeadingElectronsJets(year_str, allnamesWithPrefixFinal, filenameTree, maxevent);
